...
// $jwt = mysqli_real_escape_string($con, $data->jwt);


$validate_response =  json_decode(validateToken($key,$_COOKIE['jwtPHP']));
// $validate_response =  json_decode(validateToken($jwt,$key));
...
WHERE indicators.year_indicator = $i AND indicators.deleted = 0 ORDER BY indicators.date_encoded DESC ";
...
if(mysqli_num_rows($indicators_result) != 0) {
	while($indicators_row = mysqli_fetch_assoc($indicators_result)) {
		...
		$titlseind =  mysqli_real_escape_string($con,$indicators_row['Title']);
		...
		$target_result = mysqli_query($con, $target_query);
			if(mysqli_num_rows($target_result) != 0) {
				while($target_row = mysqli_fetch_assoc($target_result)) {
					...
					$report_query="INSERT INTO report  	(idindicators,Indicator, Sub, agency_commitment, division, pap ,Accomplished,Target,AccomWeight ,TargetWeight, Percent, Slippage, year)				VALUES					('$idindicators','$titlseind','$Sub_Act_Title', '$indicators_commitment','$division','$pap','$accVal' ,'$tarVal', '$Weight', '$TarWeight', '$Percent', '$Slippage',$i)";
